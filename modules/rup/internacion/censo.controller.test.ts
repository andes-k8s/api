...
test('Censo diario - Paciente ingresa y se queda', async () => {
    ...
    await CamasEstadosController.store({
        ...
        cama: idCama }, estadoOcupada(internacion._id, cama.unidadOrganizativaOriginal, 1, 'minute'), REQMock);
...
test('Censo diario - Paciente ingresa y tiene alta', async () => {
    ...
    await CamasEstadosController.store({
        organizacion, ambito, capa,
        cama: idCama }, estadoOcupada(internacion._id, cama.unidadOrganizativaOriginal, 2, 'minute'), REQMock);

    await CamasEstadosController.store({
        organizacion, ambito, capa,
        cama: idCama }, estadoDisponible(cama.unidadOrganizativaOriginal, 1, 'minute'), REQMock);
